...
class ProductAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if match:
            ...
            if product:
                xml_elem = dict_to_xml("product", product)
                self.wfile.write(prettify(xml_elem))
            else:
                error_xml = dict_to_xml("error", {"message": "Product not found"})
                self.wfile.write(prettify(error_xml))
                
        else:
            self.send_response(404)
            self.send_header("Content-type", "application/xml")
            self.end_headers()
            self.wfile.write(prettify(dict_to_xml("error", {"message": "Invalid endpoint"})))
